  ...
  <div class="flex h-[calc(100vh-68px)] bg-white overflow-hidden font-sans">
    ...
    <div :class="sidebarCollapsed ? 'w-0 overflow-hidden' : 'w-80'" class="flex-shrink-0 border-r border-gray-200 flex flex-col transition-all duration-200">
      <div class="p-6 border-b border-gray-100">
        ...
        <div class="relative group">
          <input
            ...
            class="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:bg-white transition-all"
          />
          <svg class="w-4 h-4 text-gray-400 absolute left-3 top-3 group-focus-within:text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-else-if="filteredContacts.length === 0" class="flex flex-col items-center justify-center flex-1 text-center px-6">
        ...
        <p class="text-xs text-gray-300 mt-1">Học viên sẽ xuất hiện khi họ nhắn tin với bạn</p>
      </div>
      <div v-else class="flex-1 overflow-y-auto divide-y divide-gray-50">
        <div
          ...
          :class="selectedContact?.userId === contact.userId ? 'bg-purple-50' : 'hover:bg-gray-50'"
        >
          <div v-if="selectedContact?.userId === contact.userId"
            class="absolute left-0 top-0 bottom-0 w-1 bg-purple-600"></div>

          <div class="relative flex-shrink-0">
            <div class="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-sm shadow-sm uppercase">
            ...
            <div class="flex justify-between items-baseline mb-1">
              ...
              <div class="flex items-center gap-1.5 flex-shrink-0 ml-1">
                ...
                <span v-if="contact.unreadCount > 0"
                  class="min-w-[18px] h-[18px] px-1 bg-purple-600 text-white text-[10px] font-black rounded-full flex items-center justify-center">
    ...
    <div v-if="selectedContact" class="flex-1 flex flex-col bg-gray-50/30">
      <div class="px-6 py-4 bg-white border-b border-gray-200 flex justify-between items-center shadow-sm z-10">
        ...
        <div class="flex items-center gap-3">
          ...
          <button @click="sidebarCollapsed = !sidebarCollapsed" class="text-gray-400 hover:text-purple-500 transition" :title="sidebarCollapsed ? 'Hiện danh sách' : 'Ẩn danh sách'">
          ...
          <button @click="toggleSearch" class="text-gray-400 hover:text-purple-500 transition" title="Tìm kiếm tin nhắn">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
            </svg>
          </button>
      ...
      <div class="relative flex-1 min-h-0">
        <div ref="messageBox" @scroll="onScroll" class="h-full overflow-y-auto p-8 space-y-6 flex flex-col">
          ...
          <template v-for="msg in messages" :key="msg.id">
            ...
            <div v-if="msg.senderId === currentUserId" :data-msg-id="msg.id" class="flex items-start gap-2 flex-row-reverse self-end max-w-[80%] animate-fade-in group/msg">
              <div class="w-8 h-8 rounded-full bg-purple-600 flex-shrink-0 flex items-center justify-center text-white text-[10px] font-bold shadow-sm uppercase">ME</div>
              <div class="flex flex-col items-end gap-1">
                <div :class="msg.deleted ? 'bg-gray-200' : 'bg-purple-600'" class="p-4 rounded-2xl rounded-tr-none shadow-md">
                  ...
                  <a v-else-if="msg.attachmentType === 'file' && msg.attachmentUrl" :href="msg.attachmentUrl" target="_blank"
                    class="flex items-center gap-1.5 text-purple-100 hover:text-white text-xs mb-2 underline">
                  ...
                  <span :class="msg.deleted ? 'text-gray-400' : 'text-purple-200'" class="text-[9px] mt-1 block text-right">{{ formatTime(msg.createdAt) }}</span>
                </div>
            ...
            <!-- Tin nhắn của học viên (trái) -->
            <div v-else :data-msg-id="msg.id" class="flex items-start gap-3 max-w-[80%] animate-fade-in">
              <div class="w-8 h-8 rounded-full bg-blue-600 flex-shrink-0 flex items-center justify-center text-white text-xs font-bold uppercase shadow-sm">
                {{ selectedContact.userName?.charAt(0) }}
              </div>
              <div :class="msg.deleted ? 'bg-gray-100' : 'bg-white'" class="p-4 rounded-2xl rounded-tl-none shadow-sm border border-gray-100">
                <img v-if="msg.attachmentType === 'image' && msg.attachmentUrl" :src="msg.attachmentUrl"
                  class="max-w-[220px] rounded-lg mb-2 cursor-pointer" @click="openImage(msg.attachmentUrl)"/>
                <a v-else-if="msg.attachmentType === 'file' && msg.attachmentUrl" :href="msg.attachmentUrl" target="_blank"
                  class="flex items-center gap-1.5 text-purple-600 hover:text-purple-800 text-xs mb-2 underline">
                  <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                  </svg>
                  {{ msg.attachmentName || 'Tải file' }}
        ...
        <button
          ...
          class="absolute bottom-4 right-4 w-9 h-9 rounded-full bg-purple-600 text-white shadow-lg flex items-center justify-center hover:bg-purple-700 transition z-10"
          title="Xuống cuối"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="p-6 bg-white border-t border-gray-100">
        <!-- Preview attachment -->
        <div v-if="pendingAttachment" class="mb-3 flex items-center gap-2 bg-purple-50 border border-purple-200 rounded-lg px-3 py-2">
          <img v-if="pendingAttachment.type === 'image'" :src="pendingAttachment.previewUrl" class="w-12 h-12 object-cover rounded"/>
          <svg v-else class="w-8 h-8 text-purple-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
          </svg>
        ...
        <form @submit.prevent="sendMessage"
          class="flex items-center gap-4 bg-gray-50 p-2 rounded-xl border border-gray-200 focus-within:border-purple-300 focus-within:ring-2 focus-within:ring-purple-100 transition-all">
          <!-- Emoji -->
          <div class="relative flex-shrink-0">
          ...
          <!-- File attach -->
          <label class="cursor-pointer text-gray-400 hover:text-purple-500 transition flex-shrink-0 px-1" title="Đính kèm file/ảnh">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            class="bg-purple-600 text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-purple-700 shadow-sm transition disabled:opacity-50">
            {{ sending ? '...' : 'Gửi đi' }}
          </button>
        </form>
      </div>
    ...
    <!-- Empty state -->
    <div v-else class="flex-1 flex flex-col items-center justify-center bg-gray-50/30 text-center p-8">
      ...
      <h3 class="text-lg font-bold text-gray-900 mb-2">Chọn một học viên</h3>
...
const fetchContacts = async () => {
  ...
  try {
    const res = await axiosClient.get('/messages/instructor/contacts');
    contacts.value = res || [];
  } catch (e) { console.error('Lỗi tải danh sách chat:', e); }
  finally { loadingContacts.value = false; }
};

// Refresh nhẹ trạng thái online (không làm flicker UI, không ghi đè unreadCount đang chọn)
const refreshPresence = async () => {
  try {
    const res = await axiosClient.get('/messages/instructor/contacts');
    const fresh = res || [];
    fresh.forEach(f => {
      const c = contacts.value.find(x => x.userId === f.userId);
      if (c) {
        c.online = f.online;
      } else {
        contacts.value.push(f);
      }
    });
    if (selectedContact.value) {
      const sc = contacts.value.find(x => x.userId === selectedContact.value.userId);
      if (sc) selectedContact.value.online = sc.online;
    }
  } catch {}
};
...
const selectContact = async (contact) => {
  ...
  try {
    ...
    // Fallback: thử endpoint không phân trang
    try {
      const res2 = await axiosClient.get(`/messages/history/${contact.userId}`);
      messages.value = res2 || [];
      hasMoreMessages.value = false;
      scrollToBottom();
    } catch { console.error('Lỗi tải tin nhắn:', e); }
  }
...
const sendMessage = async () => {
  ...
  if (connected.value) {
    ...
    try {
      ...
      scrollToBottom();
    } catch { window.__notify?.auto?.('Không thể gửi tin nhắn!'); }
